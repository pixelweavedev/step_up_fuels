...
class LedgerDao extends DatabaseAccessor<AppDatabase> with _$LedgerDaoMixin {
  ...
  Future<List<LedgerAccountRow>> getAllAccounts({
    ...
    if (searchQuery != null && searchQuery.trim().isNotEmpty) {
      ...
      list = list.where((acc) =>
          ...
          acc.accountCode.toLowerCase().contains(q)).toList();
    }
  ...
  Future<LedgerAccountRow?> getAccountById(String id) async {
    return (select(ledgerAccounts)..where((t) => t.id.equals(id))).getSingleOrNull();
  }

  Future<LedgerAccountRow?> getAccountByCode(String code) async {
    return (select(ledgerAccounts)..where((t) => t.accountCode.equals(code))).getSingleOrNull();
  }

  Future<LedgerAccountRow?> getAccountByReference(String referenceId, String referenceType) async {
    return (select(ledgerAccounts)
          ..where((t) => t.referenceId.equals(referenceId) & t.referenceType.equals(referenceType)))
        .getSingleOrNull();
  }
  ...
  Future<List<LedgerEntryRow>> getEntriesForAccount(
    ...
    final query = select(ledgerEntries)..where((t) => t.ledgerAccountId.equals(accountId));
    if (fromDate != null) {
    ...
    query.orderBy([(t) => OrderingTerm.asc(t.entryDate), (t) => OrderingTerm.asc(t.createdAt)]);
    return query.get();
  }
  ...
  Future<void> saveEntry(LedgerEntriesCompanion companion) async {
    ...
    final latest = await (select(ledgerEntries)
          ..where((t) => t.ledgerAccountId.equals(accountId))
          ..orderBy([(t) => OrderingTerm.desc(t.entryDate), (t) => OrderingTerm.desc(t.createdAt)])
          ..limit(1))
        .getSingleOrNull();
    ...
    final isAssetOrExpense = account.accountType == 'CUSTOMER' ||
