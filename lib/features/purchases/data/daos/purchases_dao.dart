...
class PurchasesDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<List<SupplierRow>> getAllSuppliers({
  ...
  Future<List<FuelPurchaseRow>> getAllPurchases({
    ...
    query.where((t) {
      ...
      if (fromDate != null) {
        expr = expr & t.purchaseDate.isBiggerOrEqualValue(fromDate);
      }
      if (toDate != null) {
        expr = expr & t.purchaseDate.isSmallerOrEqualValue(toDate);
      }
      return expr;
    });
  ...
  Future<List<FuelPurchaseItemRow>> getItemsByPurchaseId(
    String purchaseId,
  ) async {
  ...
  Future<void> deleteItemsByPurchaseId(String purchaseId) async {
    await (delete(
      fuelPurchaseItems,
    )..where((t) => t.purchaseId.equals(purchaseId))).go();
  }
  ...
  Future<int> readAndIncrementCounter() async {
    final row = await (select(
      db.appSettings,
    )..where((t) => t.key.equals('purchase_counter'))).getSingleOrNull();
