...
class PurchaseRepositoryImpl implements PurchaseRepository {
  PurchaseRepositoryImpl(this._dao);
  final PurchasesDao _dao;
  final _uuid = const Uuid();

  @override
  Future<Result<List<Supplier>>> getSuppliers({bool includeDeleted = false}) async {
    try {
  ...
  Future<Result<FuelPurchase>> getPurchaseById(String id) async {
    try {
      ...
      if (row == null) {
        return const Result.failure(DatabaseFailure(message: 'Purchase not found'));
      }
  ...
  @override
  Future<Result<List<FuelPurchaseItem>>> getPurchaseItems(String purchaseId) async {
    try {
  ...
  @override
  Future<Result<void>> savePurchase(FuelPurchase purchase, List<FuelPurchaseItem> items) async {
    try {
      ...
      await db.transaction(() async {
        ...
        final purchaseToSave = purchase.copyWith(purchaseNumber: purchaseNumber);
        ...
        if (isNew) {
          ...
          if (mainLoc == null) {
            ...
            await db.into(db.storageLocations).insertOnConflictUpdate(
          ...
          for (final item in items) {
            final movementCompanion = InventoryMovementsCompanion(
              ...
              notes: Value('Fuel Purchase: $purchaseNumber from ${purchase.supplierId}'),
              createdAt: Value(DateTime.now()),
              createdBy: Value(purchase.createdBy),
            );
            await db.into(db.inventoryMovements).insertOnConflictUpdate(movementCompanion);
          }
        }
      });
      return const Result.success(null);
