...
class ExpenseExportAdapter extends ExportAdapter<Expense> {
  ...
  Future<List<Expense>> fetchData(ExportFilter filter) async {
    ...
    final result = await useCase(
      ...
      category: filter
          .status, // We repurpose status filter for category filter in Expense if desired
    );
    return result.when(
      success: (list) {
        var filtered = list;
        if (filter.driverId != null) {
          filtered = filtered
              .where((e) => e.driverId == filter.driverId)
              .toList();
        }
        if (filter.vehicleId != null) {
          filtered = filtered
              .where((e) => e.vehicleId == filter.vehicleId)
              .toList();
        }
        return filtered;
      },
  ...
  List<DependencyRef> getDependencies(Map<String, dynamic> json) {
    ...
    if (vId != null && vId.trim().isNotEmpty) {
      refs.add(
        DependencyRef(field: 'vehicle_id', entityType: 'Vehicle', refId: vId),
      );
    }
    if (dId != null && dId.trim().isNotEmpty) {
      refs.add(
        DependencyRef(field: 'driver_id', entityType: 'Driver', refId: dId),
      );
    }
