...
class VehicleExportAdapter extends ExportAdapter<Vehicle> {
    ...
    return result.when(
      success: (list) {
        ...
        if (filter.activeOnly) {
          filtered = filtered.where((v) => v.status == VehicleStatus.active).toList();
        }
        return filtered;
      },
  ...
  List<ValidationError> validateRow(Map<String, dynamic> json) {
    ...
    if ((json['registration_number'] as String?)?.isEmpty ?? true) {
      errors.add(const ValidationError(field: 'registration_number', message: 'Registration Number is required'));
    }
    if ((json['model'] as String?)?.isEmpty ?? true) {
      errors.add(const ValidationError(field: 'model', message: 'Model is required'));
    }

    final capacity = double.tryParse(json['capacity']?.toString() ?? '');
    if (capacity == null) {
      errors.add(const ValidationError(field: 'capacity', message: 'Capacity must be a valid number'));
    } else if (capacity <= 0) {
      errors.add(const ValidationError(field: 'capacity', message: 'Capacity must be greater than zero'));
    }
